...
| 2026-06-17 | Host assignment uses a three-path model (self-request, direct assignment, invitation) with `host_assignment_status` state machine | Ensures hosts always explicitly accept direct assignments; request approvals skip redundant accept. Full logic in **`docs/host-logic.md`** | A-01 |
