-- Migration: Fix overlap check in generate_game_from_template.
--
-- Business rule (corrected):
--   • 'upcoming' or 'open'  → a game is already queued / accepting players → SKIP
--   • 'live' or 'paused'    → session is running → queue the NEXT upcoming game → ALLOW
--   • 'completed'/'cancelled'/NULL → game ended or no game → ALLOW
--
-- Previously the check blocked on ('upcoming','open','live','paused'), which
-- prevented the queue-builder from scheduling the next game while the current
-- session was live.
...
BEGIN
    ...
    -- Overlap check: skip only when a game is already queued / open for players.
    -- 'live' and 'paused' are running sessions — a new upcoming game may be queued.
