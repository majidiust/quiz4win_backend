-- Migration: generate_game_from_template() — use template name verbatim (no suffix).
--
-- Migration 20260607120001 accidentally reintroduced the timestamp suffix:
--   v_title := v_tpl.name || '_' || to_char(v_start_at AT TIME ZONE 'UTC', 'MMDD_HH24MI');
-- This migration reinstalls the full function body with llm_template_id support
-- (from 20260607120001) but sets the title directly from the template name:
--   INSERT ... title = v_tpl.name ...
-- The unused v_title variable is removed.
--
...
BEGIN
    ...
    -- Use the template name exactly — no suffix appended.
