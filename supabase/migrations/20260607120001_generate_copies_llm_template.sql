-- Migration: generate_game_from_template() copies llm_template_id.
--
-- Re-installs the canonical 20260606130000 body (overlap = ('upcoming','open');
-- cron-aligned scheduled_at; cooldown-aware picker; target_languages copy) and
-- additionally copies the template's llm_template_id onto the generated game so
-- the orchestrator's 3-tier LLM cascade (game → template → global default)
-- resolves correctly for auto-generated games.
--
-- Rule compliance: R-02/R-05 no financial columns touched; R-06 no reverse
...
BEGIN
    ...
    INSERT INTO public.games (
        ...
        v_new_game_id, v_tpl.id, v_tpl.llm_template_id, v_title, v_tpl.description, NULL,
